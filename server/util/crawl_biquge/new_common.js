  ...
  return new Promise((resolve, reject) => {
    request(url, (error, res, body) => {
      if (!error && res.statusCode == 200) {
        ...
        oneBook.isSerial = $('.detail-box .info .top .fix p:nth-child(3)').text().replace('状态：', '');
        ...
        oneBook.bookType = $('.detail-box .info .top .fix p:nth-child(2)').text().replace('类别：', '');
        ...
        oneBook.source = 'biquge'; // 书籍来源，风雨小说PC端
        resolve(oneBook)
      } else {
        reject('获取书籍基本信息失败！')
      }
...
function saveChapters(chapters, bookId) {
  return new Promise(async (resolve, reject) => {
    ...
    let editSql = `insert into book_${bookId} (${str1}) values ? on duplicate key update ${updates}`;
...
function saveContents(results, bookId) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let editSql = `insert into book_${bookId} (${str1}) values ? on duplicate key update cpContent = values(cpContent)`;
